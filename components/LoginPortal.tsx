...
import { PlusIcon, SearchIcon, TrashIcon, MinusIcon, ChevronDownIcon } from './icons';
import Shipping from './Shipping';
import LocationTag from './LocationTag';
import AccessibleNumberInput from './AccessibleNumberInput';

const SESSION_STORAGE_key = 'inventory_portal_user';
...
const LoginPortal: React.FC = () => {
        ...
        const initializePortal = async () => {
            ...
            try {
                ...
                if (savedUserJson) {
                    ...
                    // Fetch data needed for all tabs
                    const [productsData, locationsData, ordersData, appSheetData] = await Promise.all([
                        getProducts(),
                        getLocations(),
                        getLocationOrders(),
                        getAppSheetProducts(),
                    ...
                    // If not logged in, just fetch users for the login form
                    const usersData = await getUsers();
                    setUsers(usersData);
                }
            } catch (error) {
                console.error("Failed to initialize portal:", error);
                setLoginError("Could not load necessary data. Please try again later.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        const foundUser = users.find(user => user.accessCode.trim() === accessCode.trim());

        if (foundUser) {
            sessionStorage.setItem(SESSION_STORAGE_key, JSON.stringify(foundUser));
            setCurrentUser(foundUser);
            // Pre-fetch data needed for the next screen
            setIsLoading(true);
            const [productsData, locationsData, ordersData, appSheetData] = await Promise.all([
                getProducts(),
                getLocations(),
    ...
    const handleLogout = () => {
        ...
        // Refetch users for login screen
    ...
    const handleSubmitOrder = async () => {
        if (!currentUser || selectedLocations.length === 0 || orderItems.length === 0) {
    ...
    const userOrders = useMemo(() => {
        if (!currentUser || !allLocationOrders) {
            return [];
        }
        ...
        return allLocationOrders
            .filter(order => {
                if (order.createdBy !== currentUser.userID) return false;
                try {
        ...
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
                    <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Manager Login Portal</h2>
    ...
    const userLocations = currentUser.location ? currentUser.location.split(',').map(l => l.trim()).filter(Boolean) : [];

    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold text-gray-900">Manager Portal</h2>
            ...
            <div className="pt-4">
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-md sticky top-8">
                                ...
                                {submissionStatus.message && (
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-indigo-600 text-white py-3 rounded-md shadow-sm hover:bg-indigo-700 disabled:bg-indigo-400 disabled:cursor-not-allowed transition-colors font-semibold"
                                    >
                ...
                {activeTab === 'myOrders' && (
                                    ...
                                    <details key={location} open className="bg-gray-50 rounded-lg border border-gray-200">
                                        ...
                                        <div className="pt-0 p-4 space-y-4">
                                            {Object.keys(groupedUserOrders[location]).sort().reverse().map(date => {
                                                ...
                                                return (
                                                <div key={date} className="pl-4 border-l-2 border-indigo-200">
                                                    <h4 className="font-semibold text-gray-700 text-md mb-3 -ml-4 pl-3 bg-gray-200/50 py-1 rounded-r-md">{formattedDate}</h4>
                                                    <div className="space-y-3">
                                                      {Object.entries(itemsGroupedByName)
                                                        .sort(([nameA], [nameB]) => nameA.localeCompare(nameB))
                                                        .map(([itemName, group]) => (
                                                            <div key={itemName} className="bg-white border rounded-lg p-3 shadow-sm">
                                                                <div className="flex justify-between items-center">
                                                                    <h5 className="font-bold text-indigo-700">{itemName}</h5>
                                                                    <span className="px-2 py-1 text-xs font-semibold leading-5 rounded-full bg-indigo-100 text-indigo-800">
                 ...
                 {activeTab === 'shipments' && (
                    <Shipping currentUser={currentUser} allLocations={allLocations} />
                )}
            </div>
        </div>
    );
