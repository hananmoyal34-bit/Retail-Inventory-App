...
const ManagerPortal: React.FC<ManagerPortalProps> = ({ user, onLogout }) => {
    ...
    // Helper for user locations
    const userLocations = useMemo(() => 
        user.location ? user.location.split(',').map(l => l.trim()).filter(Boolean) : [], 
    [user.location]);
    ...
    // Default selection: If >1 location, select none. If 1, select it.
    const [selectedLocations, setSelectedLocations] = useState<string[]>(() => 
        userLocations.length === 1 ? userLocations : []
    );
    ...
    // Draft key unique to user
    const DRAFT_KEY = `manager_draft_order_${user.userID}`;
    ...
    useEffect(() => {
        ...
        if (history) {
            try {
                ...
                console.error("Failed to parse custom item history");
            }
        }
    }, [CUSTOM_HISTORY_KEY]);
    ...
    useEffect(() => {
        ...
        if (savedDraft) {
            try {
                const draft: DraftOrder = JSON.parse(savedDraft);
                // Check if draft is less than 24 hours old
                if (Date.now() - draft.timestamp < 24 * 60 * 60 * 1000) {
                    if (draft.items.length > 0 || draft.locations.length > 0) {
                        setOrderItems(draft.items);
                        setSelectedLocations(draft.locations);
                        setDraftRestoredMessage('Unfinished draft restored.');
                        setTimeout(() => setDraftRestoredMessage(''), 5000);
                    }
                } else {
    ...
    useEffect(() => {
        ...
        // Check if anything is different from initial state
        const initialLocations = userLocations.length === 1 ? userLocations : [];
        const locationsChanged = selectedLocations.length !== initialLocations.length || !selectedLocations.every(l => initialLocations.includes(l));
        
        const hasData = orderItems.length > 0 || locationsChanged;
        ...
        <div className="min-h-screen font-sans text-gray-800 pb-24 md:pb-0">
            ...
            <div className="space-y-6">
                ...
                {draftRestoredMessage && (
                    <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 rounded shadow-sm" role="alert">
                        <p>{draftRestoredMessage}</p>
                    </div>
                )}
                            ...
                            <div className="lg:col-span-1">
                                <div className="bg-white p-6 rounded-lg shadow-md lg:sticky top-24">
                                    ...
                                    {submissionStatus.type === 'error' && submissionStatus.message && (
                                        ...
                                        {orderItems.length > 0 && (
                                            <button onClick={handleDiscardDraft} className="w-full text-center text-xs text-gray-500 hover:text-red-600 mt-3 underline">
                                                Discard Draft
                                            </button>
                                        )}
                                    </div>
